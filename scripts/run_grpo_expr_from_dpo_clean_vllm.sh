...
if not (Version("0.12.0") <= v <= Version("0.18.0")):
    msg = (
        ...
        "本环境已通过 1-step smoke，但如果运行异常请改用 scripts/run_grpo_expr_from_dpo_clean_fast.sh。"
    )
...
$PYTHON -m src.training.grpo_expr_trainer --config configs/grpo_expr_from_dpo_clean_vllm.yaml

echo "Checkpoint: outputs/checkpoints/grpo_expr_from_dpo_clean_vllm/best"
